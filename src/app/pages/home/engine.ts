...
import { Human, ObjectBase } from './objects';
import { Process } from 'src/shared/page';

export type AppEngineEvents =
  | { type: 'hover-human'; human: Human }
  | { type: 'scene-change'; scene: SceneBase };
...
export class AppEngine extends Process {
  ...
  event$ = new Subject<AppEngineEvents>();
}
  ...
  moveCamera(camName = 'main', target: Vector3, center?: Vector3) {
    ...
    return new Observable<{ kind: 'end' | 'loop'; [key: string]: any }>(
      (suber) => {
        ...
        const oldPosA = new Vector3(
          Number(oldPos.x.toFixed(2)),
          Number(oldPos.y.toFixed(2)),
          Number(oldPos.z.toFixed(2))
        );
        if (oldPosA.equals(target)) {
          if (!center || center.equals(oldCnt)) {
            return suber.complete();
          }
        }
